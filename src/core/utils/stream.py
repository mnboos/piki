...
def run_object_detection(
    ...
    try:
        ...
        if len(all_detections) > 1:
            ...
            if _profile:
                # Log the pre-merge candidates (label, conf, normalized bbox
                # [ymin,xmin,ymax,xmax]) so any surviving duplicates can be
                # inspected offline (overlap, why they didn't merge).
                logger.info(
                    "PERF stage=pre_merge n=%d cand=%s", n_before,
                    [(d.label, round(float(d.confidence), 3),
                      [round(float(v), 3) for v in d.bbox]) for d in all_detections],
                )
